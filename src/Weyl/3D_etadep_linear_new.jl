...
function main(arg::Array{String,1})
    ...
    save_data = DataFrame(eta=mu0, Drude=Drude_mu, BCD=BC_mu, dQM=dQM_mu, Green_Dr=app_Drude0_mu, app_QM=app_dQM_mu, Green_tot=Green_mu)
    #「./」で現在の(tutorial.ipynbがある)ディレクトリにファイルを作成の意味、指定すれば別のディレクトリにファイルを作ることも出来る。
    CSV.write("./eta_dep_ZZ.csv", save_data)

    #gr()
    ENV["GKSwstype"]="nul"
    p1=plot(mu0, Drude_mu, label="Drude",xlabel="η",ylabel="σ",title="linear conductivity", width=2.0, marker=:circle)
